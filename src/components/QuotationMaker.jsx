...
import { useQuotation, QuotationProvider, ErrorBoundary } from '../context/QuotationContext';
import { Save, Download, Printer, FileText, ChevronLeft, PlusCircle } from 'lucide-react';
...
const QuotationMakerContent = () => {
  const { printMode, togglePrintMode, selectedParty, setSelectedParty, loadPartyQuotations } = useQuotation();
  ...
  useEffect(() => {
    const loadPartyData = async () => {
      ...
      if (loadingPartyRef.current || (partyId && loadedPartyIdRef.current === partyId)) {
      ...
      if (partyId) {
        try {
          ...
          if (!selectedParty || selectedParty._id !== party._id) {
            console.log(`Setting selected party to: ${party.name} (${party._id})`);
    ...
    <div className="container mx-auto p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8">
        <div className="flex items-center mb-4 md:mb-0">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">EmpressPC Quotation Maker</h1>
            {selectedParty && (
              <p className="text-gray-300 mt-1">Creating quotation for: {selectedParty.name}</p>
            )}
          </div>
        </div>
      ...
      {!selectedParty && (
        <div className="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4 rounded shadow-md my-4">
          ...
          <p>Please select a party from the search above to create a quotation.</p>
          <button 
            onClick={handleBackToParties}
